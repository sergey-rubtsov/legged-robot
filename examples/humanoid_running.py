...
p.loadSDF("kinematic/stadium.sdf")
#p.loadURDF("plane.urdf")

objs = p.loadMJCF("kinematic/humanoid_symmetric_no_ground.xml",
...
class SmallReactivePolicy(object):
  ...
  def __init__(self, name, ob_space, ac_space):
    ...
    with tf.compat.v1.variable_scope(name):
      ...
      self.ob = tf.compat.v1.placeholder(dtype=tf.compat.v1.float32, shape=batch_of_observations, name="ob")
      x = self.ob

      width = 256
      self.dense1_w = tf.compat.v1.get_variable("dense1_w", [x.get_shape()[1], width])
      self.dense1_b = tf.compat.v1.get_variable("dense1_b", [width])
      x = tf.compat.v1.nn.relu(tf.matmul(x, self.dense1_w) + self.dense1_b)

      width = 128
      self.dense2_w = tf.compat.v1.get_variable("dense2_w", [x.get_shape()[1], width])
      self.dense2_b = tf.compat.v1.get_variable("dense2_b", [width])
      x = tf.compat.v1.nn.relu(tf.compat.v1.matmul(x, self.dense2_w) + self.dense2_b)

      assert (len(ac_space.shape) == 1)
      width = ac_space.shape[0]
      self.final_w = tf.compat.v1.get_variable("final_w", [x.get_shape()[1], width])
      self.final_b = tf.compat.v1.get_variable("final_b", [width])
      x = tf.compat.v1.matmul(x, self.final_w) + self.final_b
      self.action_node = x

      self.all_variables = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES,
                                             tf.compat.v1.get_variable_scope().name)

  def act(self, ob):
    a = tf.compat.v1.get_default_session().run(self.action_node, feed_dict={self.ob: ob[None]})
    return a[0]
  ...
  def load_weights(self):
    ...
    for var, w in assigns:
      ph = tf.compat.v1.placeholder(tf.float32, w.shape)
      to_run.append(tf.compat.v1.assign(var, ph))
      feed_dict[ph] = w
    tf.compat.v1.get_default_session().run(to_run, feed_dict=feed_dict)
